...
generate_simulation_splatter <- function(dropout_index, seed_value, nGenes = 800, nbulk = 10, ncellsperbulk = 100,dropout_mid = c(4, 5, 5.5)){
  ...
  data_dropout[matrix(counts(sim) == 0)] = 0
  ...
  # og scrabble average across bulk samples (here represented by splatter batches)
  data_bulk_mean = data.frame(val = rowMeans(data_true))
  ...
  data_bulk = aggregate(as.data.frame(t(sim@assays@data@listData$TrueCounts)),list(sim@colData$Batch),mean)
  rownames(data_bulk) = data_bulk[,1]
  data_bulk = data_bulk[,-1]
  data_bulk = t(data_bulk) # ensure genes x samples orientation
...
generate_save_data <- function(dropout_index, seed_value, nGenes = 800, nbulk = 10, ncellsperbulk = 100,dropout_mid = c(4, 5, 5.5)){
...
######## SCRABBLE OG ##############
run_scrabble <- function(dropout_index, seed_value,parameter = c(1, 1e-06, 1e-04)){
  ...
  # write the data
  scrabble_path = paste0(path, "scrabble_",dropout_index,"_",seed_value,".csv")
  write.table(result,
              scrabble_path,
              sep=',',
              row.names = F,
...
######## SCRABBLE Multitask ##############
run_scrabble_m <- function(dropout_index, seed_value,parameter = c(1, 1e-06, 1e-04)){
  # Parameter in the function
  # dropout_index: the index of dropout_mid to control the dropout rate
  # seed_value: the random seed
  ...
  path = "imputation_scrabble_m_data/"
  ...
  # write the data
  # write the data
  scrabble_path = paste0(path, "scrabble_m_",dropout_index,"_",seed_value,".csv")
  write.table(result,
              scrabble_path,
              sep=',',
              row.names = F,
...
run_error <- function(dropout_index, seed_value){
  ...
  error[4] = norm(log10(data_scrabble_m + 1) - log10(data_true + 1), type = "2")
  ...
  error_cell[4] = calculate_similarity(data_true_cell, data_scrabble_m_cell)
  ...
  error_gene[4] = calculate_similarity(data_true_gene, data_scrabble_m_gene)
...
plot_meansd <- function(dropout_index, seed_value){
  ...
  # load the mtSCRABBLE results
  data = read.csv(paste0("imputation_scrabble_m_data/scrabble_m_",
                         dropout_index,"_",
                         seed_value,".csv"),
                  header = FALSE,
                  sep = "\t")
  ...
  data_scrabble_m = as.matrix(data)
  ...
  p = meanSdPlot(log10(data_scrabble_m + 1), ranks = FALSE)

  pl[[5]] = p$gg + ggtitle("mtSCRABBLE")
...
plot_comparison_tsne <- function(dropout_index,
  ...
  # calculate the tsne of mtSCRABBLE imputed data
  scrabble_m_tsne = Rtsne(t(as.matrix(data_scrabble_m)),
                     initial_dims = initial_dims_value,
                     perplexity = perplexity_value)

  pl[[5]] = plot_pca_singlecell(scrabble_m_tsne$Y,data_simulation$group)
...
plot_ma <- function(dropout_index, seed_value){
  ...
  # mtSCRABBLE
  pl[[4]] = prepare_ma_data_plot(data_true, as.matrix(data_scrabble_m), "mtSCRABBLE")
...
get_cor_data <- function(dropout_index, seed_value){
  ...
  data_cell[[5]] = data_scrabble_m_cell
  ...
  data_gene[[5]] = data_scrabble_m_gene
