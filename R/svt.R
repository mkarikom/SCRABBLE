...
svt <- function(data, lambda = NaN, k = 5, incre = 5, m = NaN, n = NaN,
  ...
  print(paste0('SVT init: ',init_end_time-init_start_time))
  ...
  while(iter > 0){
    ...
    # nground <- length(e)
    nground <- 0
    eigs_start_time = Sys.time()
    # value_svt_tmp <- rARPACK::eigs(matvec, n_eig*k + nground, n = m+n, args = data, which = "LR")
    value_svt_tmp <- rARPACK::eigs(matvec, n_eig*k + nground, n = m+n, args = data, which = "LR", opts=list(maxitr=10000))
    eigs_end_time = Sys.time()
    # print(paste0('SVT eigs, i=', iter,': ',eigs_end_time-eigs_start_time))

    iterfinish_start_time = Sys.time()
    ...
    iterfinish_end_time = Sys.time()
    # print(paste0('iter finish, i=', iter,': ',iterfinish_end_time-iterfinish_start_time))
  }
