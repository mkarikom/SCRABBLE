...
// [[Rcpp::export]]
Eigen::MatrixXd cDescent(SEXP gamma_Y_B_Lambdas, SEXP As, SEXP zoness, SEXP newXs, SEXP nIters, SEXP nSDCIters, SEXP error_thresholds, SEXP iOuters){
  double error_threshold = Rcpp::as<double>(error_thresholds);
  int nIter = Rcpp::as<int>(nIters);
  int iOuter = Rcpp::as<int>(iOuters);
  int nSDCIter = Rcpp::as<int>(nSDCIters);
  ...
  while( (l < nIter) && (error > error_threshold)){
    ...
    Rcout << "descent (inner) iteration : " << l << ", descent (outer) iteration : " << iOuter << ", error: " << error << "\n";
    X1 = X;
    l = l + 1;
  }
  return X;
